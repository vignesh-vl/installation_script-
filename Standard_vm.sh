...
path=`cat $pwd/property_file| grep "path"| cut -d"=" -f2 `

directory_1=`cat $pwd/property_file| grep "apigateway_directory_name_key_1"| cut -d"=" -f2 `

directory_2=`cat $pwd/property_file| grep "apigateway_directory_name_key_2"| cut -d"=" -f2 `
...
sudo chmod 777 $directory_1

sudo chmod 777 $directory_2
...
sudo sed -i "s+^InstallDir=.*+InstallDir=$path/$directory_1+" $pwd/files/apigateway1_script


sudo sed -i "s+^InstallDir=.*+InstallDir=$path/$directory_2+" $pwd/files/apigateway2_script


sudo sed -i "s+^InstallDir=.*+InstallDir=$path/elastic_search+" $pwd/files/elastic_search_script
...
sudo sed -i "s+^integrationServer.LicenseFile.text=__VERSION1__.*+integrationServer.LicenseFile.text=__VERSION1__,$int_path+" $pwd/files/apigateway1_script

sudo sed -i "s+^YAMLicenseChooser=__VERSION1__.*+YAMLicenseChooser=__VERSION1__,$microgateway_path+" $pwd/files/apigateway1_script

sudo sed -i "s+^integrationServer.LicenseFile.text=__VERSION1__.*+integrationServer.LicenseFile.text=__VERSION1__,$int_path+" $pwd/files/apigateway2_script

sudo sed -i "s+^YAMLicenseChooser=__VERSION1__.*+YAMLicenseChooser=__VERSION1__,$microgateway_path+" $pwd/files/apigateway2_script
...
./SoftwareAGInstaller-Linux_x86_64.bin  -readScript apigateway1_script
...
cd  $path/$directory_1/IntegrationServer/instances/default/bin/
...
cd  $path/$directory_2/IntegrationServer/instances/default/bin/
...
cd $path/elastic_search/InternalDataStore/bin/

./startup.sh

until $(curl --output /dev/null  --silent --head --fail http://localhost:9440); do
    sleep 10
done


curl --user Administrator:manage http://localhost:9440/_cluster/health -H "Accept:application/json"|jq "." > sts_check.json

if [ `jq ".status" sts_check.json` == \"green\" ]
 then 
  echo "elastic search status : `jq '.status' sts_check.json` "
  rm sts_check.json
 else 
